...
| Backup/PITR/export/restore | Phase 24C.0 DB-level proof pass; Phase 24C.1 not passed | Supabase Free/no-PITR posture remains noted. Phase 24C completed manual logical export and local Docker DB-level restore proof before real external customer data. Existing RLS suite against the restored DB failed, and app/dashboard restore smoke was not run. |
