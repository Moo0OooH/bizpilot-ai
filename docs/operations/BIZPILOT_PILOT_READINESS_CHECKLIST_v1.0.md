...
| Backup/export/restore | DB-level proof pass; strict full pass not claimed | Phase 24C completed Supabase CLI logical export, local Docker Postgres restore, sanitized table-count checks, specific `MrTester` business/lead count checks, and git dump-file exclusion proof. App/dashboard/RLS restore smoke was not run. Follow `docs/ops/BACKUP_EXPORT_RESTORE_RUNBOOK.md` for evidence. |
...
| Backup/PITR/export/restore | DB-level proof pass; strict full pass not claimed | Supabase Free/no-PITR posture remains noted. Phase 24C completed manual logical export and local Docker DB-level restore proof before real external customer data; app/dashboard/RLS restore smoke was not run. |
...
| PITR/export/restore execution | DB-level proof pass; strict full pass not claimed | Phase 24C used Supabase CLI export and existing local Docker Supabase/Postgres as disposable restore target. App/dashboard/RLS restore smoke was not run. |
