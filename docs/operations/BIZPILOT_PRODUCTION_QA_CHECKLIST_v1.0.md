...
| Public quote security | Partial route-only pass; data-bearing smoke blocked | `pnpm smoke:quote -- --base-url=https://bizpilo.com --inactive-slug=phase-21-synthetic-unavailable-check` passed and rendered the safe unavailable state without raw/internal markers. Active submission/lead/dashboard checks still require synthetic owner sessions, active slug, fake payload, and cleanup/retain-evidence decision. |
| fr-CA quote flow | Blocked | Phase 21F not run. `pnpm smoke:quote` supports an approved `--fr-slug`, but no active synthetic fr-CA production slug/session is available yet. |
| Dashboard lead workflow | Blocked | Requires a valid synthetic production quote submission and founder/owner dashboard smoke. |
| Tenant isolation production smoke | Prepared; blocked on synthetic sessions/leads | Phase 21R records the horizontal-access matrix. Requires two synthetic owner sessions/businesses and synthetic leads. Local RLS tests pass, but production browser smoke is still needed. |
