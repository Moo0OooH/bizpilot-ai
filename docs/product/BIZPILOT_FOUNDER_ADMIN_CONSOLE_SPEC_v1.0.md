...
synthetic/test auth user cleanup control
```

Full customer user administration is deferred until pilot validation proves the need. The current cleanup control is intentionally limited to synthetic/test auth identities and must block founder accounts, production-customer users, and workspace owners.
...
view usage counts
add internal note
clean up synthetic/test auth login
```
...
- Synthetic/test auth login cleanup is separate from workspace cleanup and must use double confirmation.
